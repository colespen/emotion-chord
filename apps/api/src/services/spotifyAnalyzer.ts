...
export class SpotifyAnalyzer {
  ...
  private extractEmotionKeywords(emotion: string): string {
    ...
    const emotionWords = [
      'happy', 'sad', 'angry', 'calm', 'excited', 'peaceful', 'dark', 'bright',
      'melancholy', 'joyful', 'energetic', 'somber', 'uplifting', 'dramatic',
      'tender', 'powerful', 'gentle', 'intense', 'serene', 'nostalgic',
      'transcendent', 'wonder', 'love', 'hope', 'fear', 'anxiety', 'relaxed'
    ];
  ...
  private estimateFromTrackMetadata(_tracks: any[], emotionKeywords: string) {
    console.log("Estimating audio features from track metadata");
  ...
  async searchAndAnalyze(emotion: string, limit: number = 5) {
    try {
      ...
      try {
        ...
        console.log("Attempting to get audio features individually...");
        const individualFeatures = [];
        for (const trackId of trackIds.slice(0, 3)) { // Try fewer tracks
          try {
        ...
        if (individualFeatures.every(f => f === null)) {
          console.log("Could not get any audio features, using track metadata fallback");
      ...
      const count = audioFeatures.body.audio_features.filter(track => track !== null).length;
      ...
      console.log(`Processed audio features for ${count} tracks`);
      ...
      console.log("Spotify analysis result:", result);
      return result;
    } catch (error) {
